...
- Даёт внешние HTTP‑endpoint’ы: `/send-message` для отправки уведомлений в MAX‑чат и `/send-to-group-by-chatid` для отправки сообщения в групповой чат по его ChatID.
...
### 5.3. Отправить сообщение в группу по ChatID

Имя группового чата может быть не уникальным, поэтому отправка в группу выполняется **по ChatID** (уникальный идентификатор чата). ChatID можно получить из кнопки «ID этого чата 🆔» в группе или из списка `GET /group-chats`.
...
{
  "chat_id": 174132016,
  "text": "Текст сообщения в группу"
}
```

- `chat_id` — обязательное поле, числовой идентификатор группового чата (int64).
- `text` — текст сообщения.

При успехе: `200 OK`, тело `{"status":"ok","chat_id":174132016}`.  
При ошибке (неверный JSON, пустой chat_id, ошибка API MAX): `400` с `{"status":"error","message":"..."}`.
...
Отправить сообщение в одну из групп по ChatID из списка:

```bash
curl -X POST https://maksik.ask-gps.ru/send-to-group-by-chatid \
  -H "Content-Type: application/json" \
  -d '{"chat_id": 174132016, "text": "Текст уведомления"}'
```
