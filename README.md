...
### Интеграция с бэкендом (записи на приём)

- **`APPOINTMENT_CANCEL_ENDPOINT`**: URL для отмены записи (POST с `{"appointment_id": 123}`)
- **`APPOINTMENT_DOCTORS_ENDPOINT`**: URL для получения врачей по отделению (GET с `?department=Терапия`)
- **`APPOINTMENT_BACKEND_API_KEY`**: API ключ для авторизации на бэкенде (опционально)
...
- **Приду** — подтверждение записи
- **Отменить** — отмена записи + POST-запрос на `APPOINTMENT_CANCEL_ENDPOINT`
- **Перенести** — многоэтапный flow:
  1. Выбор недели (текущая/следующая)
  2. Выбор дня недели (Пн-Пт) с конкретной датой
  3. Выбор врача — список получается с бэкенда (`APPOINTMENT_DOCTORS_ENDPOINT`) по отделению оригинальной записи
  4. Выбор времени — доступные слоты
  5. Подтверждение переноса
